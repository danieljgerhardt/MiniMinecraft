...
Wolf::Wolf(OpenGLContext *context, std::vector<glm::vec3>& vertices,
           std::vector<glm::vec3>& normals, std::vector<std::vector<int>>& faces, glm::vec3 pos, const Terrain &t)
    : EntityDrawable(context), Entity(pos), hasBeenSkinned(false), terrain(t) {
...
GLenum Wolf::drawMode() {
    return GL_TRIANGLES;
}

void Wolf::addFace(uPtr<Face> fPtr) {
    this->faces.push_back(std::move(fPtr));
}

void Wolf::addVertex(uPtr<Vertex> vPtr) {
    this->vertices.push_back(std::move(vPtr));
}

void Wolf::addHE(uPtr<HalfEdge> hePtr) {
    this->hes.push_back(std::move(hePtr));
}

bool Wolf::hasVertices() const {
    return !this->vertices.empty();
}

std::vector<uPtr<Vertex>>& Wolf::getVertices() {
    return this->vertices;
}

std::vector<uPtr<Face>>& Wolf::getFaces() {
    return this->faces;
}

std::vector<uPtr<HalfEdge>>& Wolf::getHes() {
    return this->hes;
}

void Wolf::setRootJoint(uPtr<Joint> newRoot) {
    this->joint = std::move(newRoot);
}

void Wolf::findVertJointsRec(Vertex* v, Joint* curr, std::array<Joint*, 2>& jointArr, std::array<float, 2>& dists) {
    curr->updateBindMat();
...
bool Wolf::hasJoints() const {
    return !(this->joint == nullptr);
}

uPtr<Joint>& Wolf::getRoot() {
    return this->joint;
}

bool Wolf::skinned() const {
    return this->hasBeenSkinned;
}

void Wolf::tick(float dT, qint64 currentTime) {
    this->genInputs(dT);
...
void Wolf::tick(float dT, InputBundle& input) {}

// grid marching from the slides
bool gridMarchC(glm::vec3 rayOrigin, glm::vec3 rayDirection, const Terrain &terrain, float *out_dist, glm::ivec3 *out_blockHit) {
...
void Wolf::collideAndMove(const Terrain& terrain, float dt) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        for (const glm::vec3& point : boundingBox) {
            ...
            gridMarchC(point + m_position, m_velocity[i] * dt * axisDirections[i], terrain, &collisionDist, &block);
...
void Wolf::computePhysics(float dT, const Terrain &terrain) {
...
void Wolf::genInputs(float dT) {
    ...
    glm::vec3 targetPos = m_position + (glm::length(playerPos - m_position) - MIN_DIST) * direction;
    ...
    if ((rayDir.y > 0 || glm::length(m_velocity) < 0.2) &&
        gridMarchC(m_position, m_velocity * dT, terrain, &collisionDist, &blockPos) &&
...
void Wolf::moveAlongVector(glm::vec3 dir) {
    Entity::moveAlongVector(dir);
}

void Wolf::moveForwardLocal(float amount) {
    Entity::moveForwardLocal(amount);
}
void Wolf::moveRightLocal(float amount) {
    Entity::moveRightLocal(amount);
}
void Wolf::moveUpLocal(float amount) {
    Entity::moveUpLocal(amount);
}
void Wolf::moveForwardGlobal(float amount) {
    Entity::moveForwardGlobal(amount);
}
void Wolf::moveRightGlobal(float amount) {
    Entity::moveRightGlobal(amount);
}
void Wolf::moveUpGlobal(float amount) {
    Entity::moveUpGlobal(amount);
}
void Wolf::rotateOnForwardLocal(float degrees) {
    Entity::rotateOnForwardLocal(degrees);
}
void Wolf::rotateOnRightLocal(float degrees) {
    Entity::rotateOnRightLocal(degrees);
}
void Wolf::rotateOnUpLocal(float degrees) {
    Entity::rotateOnUpLocal(degrees);
}
void Wolf::rotateOnForwardGlobal(float degrees) {
    Entity::rotateOnForwardGlobal(degrees);
}
void Wolf::rotateOnRightGlobal(float degrees) {
    Entity::rotateOnRightGlobal(degrees);
}
void Wolf::rotateOnUpGlobal(float degrees) {
    Entity::rotateOnUpGlobal(degrees);
}

void Wolf::updatePlayerPos(glm::vec3 newPos) {
...
glm::vec3 Wolf::getMPos() const {
    return this->m_position;
}

float Wolf::getMyRot() const {
    return this->whatIsMyRot;
}

bool Wolf::isMoving() const {
    return glm::length(this->m_velocity) > 0.01;
}

void Wolf::animateLegs(float currentTime) {
    currentTime /= 100;
