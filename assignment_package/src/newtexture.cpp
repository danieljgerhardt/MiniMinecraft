...
NewTexture::NewTexture(OpenGLContext *context)
...
void NewTexture::create(const char *texturePath)
{
...
void NewTexture::load(int texSlot = 0)
{
    context->printGLErrorLog();
...
void NewTexture::bind(int texSlot = 0)
{
    context->glActiveTexture(GL_TEXTURE0 + texSlot);
    context->glBindTexture(GL_TEXTURE_2D, m_textureHandle);
}

int NewTexture::getHandle() {
